...
function createNativeReadable(url, size, innertube, videoInfo) {
  ...
  return new import_stream.Readable({
    async read() {
      ...
      for await (const chunk of import_youtubei.Utils.streamToIterable(getReq.body)) {
...
var LiveChat = class {
  ...
  chatUpdateHandler = (action) => {
    if (action.is(import_youtubei2.YTNodes.AddChatItemAction)) {
      const { item } = action.as(import_youtubei2.YTNodes.AddChatItemAction);
      switch (item.type) {
...
async function stream(url, skipStream, options, tube, client) {
  ...
  const yt = tube || await import_youtubei3.default.create({
...
function defaultFetch(_player, input, init, proxy) {
  ...
  if (proxy) {
    ...
    return import_youtubei4.Platform.shim.fetch(input, requestInit);
  } else {
    return import_youtubei4.Platform.shim.fetch(input, requestInit);
  }
}
...
var YoutubeiExtractor = class _YoutubeiExtractor extends import_discord_player.BaseExtractor {
  ...
  async #createWithToken() {
    ...
    this.innerTube = await import_youtubei5.default.create({
      retrieve_player: this.options.disablePlayer === true ? false : true,
      ...this.options.innertubeConfigRaw,
      cookie: this.options.cookie,
      cache: new import_youtubei5.UniversalCache(false),
  ...
  async activate() {
    ...
    this.innerTube = await import_youtubei5.default.create({
      ...INNERTUBE_OPTIONS,
      fetch: (input, init) => defaultFetch(this.context.player, input, init, this.options.proxy),
      cache: this.options.generateWithPoToken ? new import_youtubei5.UniversalCache(false) : void 0,
    ...
    if (this.options.authentication) {
      try {
        ...
        this.context.player.debug(
          info.contents?.contents ? `Signed into YouTube using the name: ${info.contents.contents[0].is(import_youtubei5.YTNodes.AccountItem) ? info.contents.contents[0].as(import_youtubei5.YTNodes.AccountItem).account_name.text ?? "UNKNOWN ACCOUNT" : "UNKNOWN ACCOUNT"}` : `Signed into YouTube using the client name: ${this.innerTube.session.client_name}@${this.innerTube.session.client_version}`
        );
      } catch (error) {
  ...
  async handle(query, context) {
    ...
    switch (context.type) {
      case import_discord_player.QueryType.YOUTUBE_PLAYLIST: {
        ...
        if (videoId && plId) {
          const endpoint = new import_youtubei5.YTNodes.NavigationEndpoint({
            continuationCommand: {
              videoId,
              playlistId: plId
            }
